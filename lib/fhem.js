...
//console.log( 'value: ' + value );
                   ...
                   if( subscriptions ) {
                     ...
                     if( reading === 'alexaName' ) {
                       if( 0 && !connection.fhem.devices[device] ) {
                         connection.fhem.emit( 'RELOAD', device );
if( 0 ){
                         FHEM_execute(connection.log, connection, 'jsonlist2 '+ device,
                                      function(result) {
//try {
                                        var d = JSON.parse( result );
                                        if( d.totalResultsReturned === 1 ) {
                                          var device = new FHEMDevice(this, d.Results[0]);
                                          if( device && device.service_name ) {
                                            device.fhem = this.connection.fhem;
                                            this.devices.push(device);
                                            this.emit( 'RELOAD', device.name );
                                          }
                                        }
                                       
//} catch(err) {
//  connection.log.error( '  error adding new device: ' + err );
//}
                                       }.bind(connection.fhem) );
}
                       } else {
