...
function FHEM(log, config, api, startupCmd) {
  ...
  //FIXME: try websocket first
  FHEM_startLongpoll( this.connection, startupCmd );
}
...
var FHEM_csrfToken = {};
//FIXME: add filter
function FHEM_startLongpoll(connection, startupCmds) {
...
//console.log( 'value: ' + value );
             ...
             for (let i = 0; startupCmds && i < startupCmds.length; i++) {
               const job = startupCmds[i];
               connection.log.info ("Executing cmd '" + job.cmd + " from startup command list");
               FHEM_execute (connection.log, connection, job.cmd, job.callback);
            }
