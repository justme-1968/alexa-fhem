  ...
  if( !ssh_client ) {
    ...
    ssh_client.stderr.on('data', (data) => {
      ...
      if (data.toString().toLowerCase().indexOf('pseudo-terminal')<0)
        this.set_ssh_state( 'running', 'stderr='+ data.toString() );
    });

    ssh_client.on('close', (code) => {
      if( ssh_client_state !== 'running' ) {
        log.error('SSH: exited with ' + code);
        this.set_ssh_state( 'stopped' );
        return;
      }
      ...
      setTimeout(()=>{ this.open_ssh(alexaConfig) }, delay);
    });
...
Server.prototype.run = function() {
  ...
  for( let connection of this._config.connections ) {
    var fhem = new FHEM(Logger.withPrefix(connection.name), connection, undefined, startupCmds);
...
log.debug(fhem.fhemIntents);
    ...
    fhem.on( 'ATTR', function(f, device, attribute, value) {
f.log.error( device +":"+ attribute +":"+ value );
      this.postEvent({
  "event": {
    "header": {
...
const REPORT_ADDorUPDATE = "AddOrUpdateReport";
...
Server.prototype.postEvent = function(event) {
  return;
    if( event && event.event && event.event.payload && event.event.payload.endpoints && event.event.payload.endpoints[0] && event.event.payload.endpoints[0].scope )
      event.event.payload.endpoints[0].scope = event_token.access_token;
