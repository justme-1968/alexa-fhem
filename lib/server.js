...
var ssh_client;
function open_ssh(aconfig) {
  if (!ssh_client) {
    const exec = require('child_process').spawn;
    ssh_client = exec( aconfig.ssh,
      ['-R', '1234:' + aconfig['bind-ip']+ ':' + aconfig.port, '-oServerAliveInterval=90', '-p', '58824', 'fhem-va.fhem.de']);
    //     { stdio: 'pipe' });
    ...
    ssh_client.on('close', (code) => {
      ...
      setTimeout(()=>{ open_ssh(aconfig) }, delay);
    });
