...
Server.prototype.startProxy = function() {
  ...
  server.listen(this._config.sshproxy.port, this._config.sshproxy['bind-ip'], function(){
    ...
    var startupPromise = User.autoConfig(false, this._config);
...
Server.prototype.run = function() {
  ...
  const startupCmds = this._config.sshproxy ?
...
log.debug(fhem.fhemIntents);
    ...
    fhem.on( 'ATTR', function(fhem, device, attribute, value) {
      ...
      var endpoint = deviceToEndpoint( device );
      if( !endpoint )
        return;
...
var verifyToken = function(event, callback) {
  ...
  if( event.directive && event.directive.header && event.directive.header.namespace === NAMESPACE_Authorization && event.directive.header.name === "AcceptGrant" ) {
    ...
    // 2) This token is in the array of bearerToken(s) we read from FHEM (instances) - sshproxy mode.
    // TODO: Shall we also check for remoteAddr=127.0.0.1 in UC 3?
  } else if(
    (token === accepted_token.token && Date.now() < accepted_token.expires) ||
     (ptoken && published_tokens.indexOf(ptoken)>=0 && this._config.sshproxy)) {
    handler.bind(this)( event, callback );
...
function deviceToEndpoint(device) {
  //console.log(device);
