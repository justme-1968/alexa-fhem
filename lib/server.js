...
Server.prototype.open_ssh = function() {
  if (!ssh_client) {
    ...
    this.setreading(SSHSTATUS_NAME, "status:starting,txt:starting SSH");
    ...
    ssh_client.stdout.on('data', (data) => {
      if (data.toString().indexOf('Welcome at the reverse proxy')>=0)
        this.setreading(SSHSTATUS_NAME, "status:running,txt:SSH connected");
      else
        this.setreading(SSHSTATUS_NAME, "status:running,txt:" + data.toString());
    ...
    ssh_client.stderr.on('data', (data) => {
      ...
      this.setreading(SSHSTATUS_NAME, "status:running,txt:stderr=" + data.toString());
    });
    ssh_client.on('close', (code) => {
      ...
      this.setreading(SSHSTATUS_NAME, "status:stopped,txt:Terminated with " + code + ", ssh will restart at " + d.join(':'));
      setTimeout(()=>{ open_ssh(alexaConfig) }, delay);
    });
    process.on('exit', (code) => {
      this.setreading(SSHSTATUS_NAME, "status:stopping,txt:alexa-fhem terminating");
...
Server.prototype.run = function() {
  ...
  if ( this._config.alexa['publicSkill'] ) {
    ...
    startupPromise.then ( (r) => {
      ...
      setTimeout( ()=>{ this.setreading(SSHSTATUS_NAME, "status:error,txt:" + e) }, 2000);
