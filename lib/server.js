...
function propertiesFromDevice(device) {
  ...
  if( device.mappings[ColorTemperature] || device.mappings[FHEM.CustomUUIDs.ColorTemperature] || device.mappings[FHEM.CustomUUIDs.CT] ) {
    ...
    if( device.mappings[FHEM.CustomUUIDs.ColorTemperature] )
      ...
      current = parseInt(1000000 / parseFloat(device.fhem.cached(device.mappings[ColorTemperature].informId)) );
...
var handleControlSetColorTemperature = function(event) {
  ...
  if( device.mappings[FHEM.CustomUUIDs.ColorTemperature] )
    ...
    device.command( device.mappings[ColorTemperature], 1000000 / target );
  else if( device.mappings[FHEM.CustomUUIDs.CT] )
