...
Server.prototype.unregister_ssh = function() {
  ...
  const prc = execSync( proxy_config.ssh, ['-p', '58824', 'fhem-va.fhem.de', 'unregister']);
  if (prc.error) {
...
Server.prototype.open_ssh = function() {
  if( !ssh_client ) {
    ...
    ssh_client = exec( proxy_config.ssh,
      ['-R', '1234:' + proxy_config['bind-ip']+ ':' + proxy_config.port, '-oServerAliveInterval=90', '-p', '58824',
        '-i', '~/.ssh/id_rsa', 'fhem-va.fhem.de']);
    //     { stdio: 'pipe' });
