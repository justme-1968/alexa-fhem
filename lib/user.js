...
User.autoConfig = function (interactive, config, fhemconn, alexaDevName) {
  return new Promise(function (resolve, reject) {
    (async () => {
      let finalError;
      finalError = await runAutoconfig(interactive, config, fhemconn, alexaDevName).catch((r)=>{finalError=r;});
      if (finalError && typeof finalError === "string") {
        log.error("sshautoconf: aborted with " + finalError);
        reject(finalError);
      } else {
...
async function runAutoconfig(interactive, config, fhemconn, alexaDevName) {
  ...
  if (! config) {
    ...
    while (retry) {
      ...
      if (!testReq.success) {
        ...
        writeout("FHEM-Connectivity fine, CSRF-Token: " + csrfToken);
        retry = false;
      }
    }
    if (failmsg)
  ...
  if (fs.existsSync(pubkey_path)) {
    ...
    if (s_json.totalResultsReturned < 1) {
      await FHEM_execute(fhemconn, "define MyAlexa alexa");
      ...
      if (goon !== 'n') {
        ...
        try {
          ...
          if (rollback && registrationKey) {
            ...
            const prc = execSync(config.sshproxy.ssh, ['-p', 58824, 'fhem-va.fhem.de', 'unregister'], {stdio: 'pipe'});
          }
        }
        if (interactive)
          ...
          setTimeout(()=>{
            FHEM_execute(fhemconn, "{ FW_directNotify(\"#FHEMWEB:WEB\", \"location.reload('true')\", \"\") }").catch(
              (e)=> { log.info("Sending reload-event failed: " + e);}
            )}, 1500);
        }
...
function
ex_FHEM_execute(log,connection,cmd,callback) {
  return new Promise((resolve, reject) => {
      ...
      .get(getRequestOptions(connection, url),
        function (err, response, result) {
          if (!err && response.statusCode === 200) {
            ...
            writeout('There was a problem connecting to FHEM (' + url + '): ' + (err ? JSON.stringify(err) : ""));
            if (response)
...
function getRequestOptions(config, url) {
  let options = use_ssl ? { rejectUnauthorized: false} : {};
  if (auth) {
...
User.auth = function(a) {
  ...
  writeout( 'error: auth format wrong. must be user:password' );
  process.exit(0);
}
