...
User.autoConfig = function (interactive, config) {
  return new Promise(function (resolve, reject) {
    (async () => {
      let finalError;
      finalError = await runAutoconfig(interactive, config).catch((r)=>{finalError=r;});
  ...
  if (! config) {
    ...
    if (!fs.existsSync(configPath)) {
      if (fs.existsSync("./config-sample.json")) {
        console.log(configPath + " not existing, creating from ../config-sample.json");
        try {
    ...
    if (!config.hasOwnProperty('connections')) {
      ...
      if (hadUid && !wasEqual) {
        console.log ("WARNING! Your userid is " + process.getuid() + ", but FHEM seems to be running with id " +
          config.connections[0].uid);
        console.log ("This is nearly guaranteed to cause problems. Please run bin/alexa -A from a shell with");
        console.log ("user-permissions, e.g. by using:");
        console.log ("  sudo -u #" + config.connections[0].uid + " /bin/bash");
    ...
    if (testReq.success)
      ...
      log.error('autoconf: testrequest result is ' + JSON.stringify(testReq));
      return 'FHEM Web not reachable - ' + (testReq.httpcode ? testReq.httpcode : "-");
    }
  }
  ...
  if (! fs.existsSync(pubkey_path)) {
    if (fs.existsSync(config.sshproxy.ssh + '-keygen')) {
      ...
      if (goon !== 'n') {
        ...
        const prc = execSync(config.sshproxy.ssh + '-keygen',  ['-t', 'rsa', '-N', '', '-f', pubkey_path], { stdio: 'pipe'});
        if (prc.error) {
  ...
  if (fs.existsSync(pubkey_path)) {
    ...
    if (!alexaDevice.Readings.hasOwnProperty(BEARERTOKEN_NAME)) {
      ...
      if (s_json.totalResultsReturned > 0) {
        ...
        if (s_json.Results[0].Readings.hasOwnProperty('bearerToken')
            && s_json.Results[0].Readings.bearerToken.Value.indexOf('crypt:') !== 0) {
          await FHEM_execute(log, conn,
            "set " + alexaDevice.Name + " proxyToken " + s_json.Results[0].Readings.bearerToken.Value);
          dirty = true;
        }
        if (s_json.Results[0].Readings.hasOwnProperty('skillRegistrationKey')
            && s_json.Results[0].Readings.skillRegistrationKey.Value.indexOf('crypt:') !== 0) {
          await FHEM_execute(log, conn,
            "set " + alexaDevice.Name + " proxyKey " + s_json.Results[0].Readings.skillRegistrationKey.Value);
          dirty = true;
        }
        if (dirty) {
    ...
    if (!alexaDevice.Readings.hasOwnProperty(BEARERTOKEN_NAME) || !sshKeyIsRegistered) {
      ...
      if (goon !== 'n') {
        ...
        await FHEM_execute(log, conn, "set " + alexaDevice.Name + " proxyToken " + bearerToken);
        await FHEM_execute(log, conn, "set " + alexaDevice.Name + " proxyKey " + registrationKey);
        await FHEM_execute(log, conn, "{ FW_directNotify(\"#FHEMWEB:WEB\", \"location.reload('true')\", \"\") }");
