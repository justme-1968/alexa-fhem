    ...
    if (!fs.existsSync(pubkey_path)) {
      if (fs.existsSync(config.sshproxy.ssh + '-keygen')) {
        ...
        if (goon !== 'n') {
          ...
          const prc = execSync(config.sshproxy.ssh + '-keygen', ['-t', 'rsa', '-b', 4096, '-N', '', '-f', pubkey_path, '-o', '-a', 100], {stdio: 'pipe'});
          //const prc = execSync(config.sshproxy.ssh + '-keygen',  ['-t', 'ed25519', '-N', '', '-f', pubkey_path, '-o', '-a', 100], { stdio: 'pipe'});
          if (prc.error) {
